...
def setup_handlers(application, user_handlers, admin_handlers, mute_service):
    # Handler temporário para logar todas as mensagens recebidas
    from telegram.ext import MessageHandler, filters
    async def log_all_messages(update, context):
        user = update.effective_user
        chat = update.effective_chat
        message = update.message
        logging.info(f"[LOG_ALL] Mensagem recebida: user_id={user.id if user else None}, username={user.username if user else None}, chat_id={chat.id if chat else None}, chat_type={chat.type if chat else None}, text={message.text if message else None}")
        print(f"[LOG_ALL] Mensagem recebida: user_id={user.id if user else None}, username={user.username if user else None}, chat_id={chat.id if chat else None}, chat_type={chat.type if chat else None}, text={message.text if message else None}")
    ...
    try:
        ...
        application.add_handler(CommandHandler("broadcast", admin_handlers.broadcast_handler))
